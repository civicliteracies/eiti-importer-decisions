...
**Decision:** A single feature flag, `accept_legacy_archive_imports` (in `shared.settings`), gates a lenient cleaner pathway. When the flag is on (the default during archive backfill), the cleaner's `MintAnonymousEntityRule` proposes a synthetic explicit placeholder for each blank identifying cell — `"Anonymous"` for the name field, paired with a sentinel `eiti_id` of the form `eiti_company_anonymous` / `eiti_project_anonymous` / `eiti_government_anonymous`. The enricher resolves the synthetic name via a pre-seeded manifest alias to the sentinel eiti_id, so import succeeds with a clearly marked anonymous row. When the flag is off (future default), the cleaner does not propose, the cell stays `BLANK_CELL_BLOCKING`, and the file requires source-side correction.
...
**Technical detail:** Flag defined at `packages/shared/src/shared/settings.py:Settings.accept_legacy_archive_imports`. Cleaner rule at `packages/cleaner/src/cleaner/rules.py:MintAnonymousEntityRule`. Sentinel aliases at `packages/enricher/src/enricher/resources/entity-aliases.toml` (entries with `eiti_id = "eiti_company_anonymous"` etc.). Emission code is `CleaningCode.FILLED_ANONYMOUS_PLACEHOLDER`. All transitional code in this scope carries the `# LEGACY_ARCHIVE_UPLOAD` marker for grep-driven cleanup.
...
For SCD2 to actually merge a submission's contribution, the family's `metadata_findings_builder` must emit a `MappingCode.CELL_MAPPED` finding targeting the entity's `id_field` (e.g. `metadata_companies.eiti_id_company`). SDF emits this in `packages/shared/src/shared/families/sdf.py::build_sdf_entity_metadata_findings`; Company Assessment emits it in `packages/shared/src/shared/families/company_assessment.py::build_company_assessment_metadata_findings`. The architectural test `tests/unit/test_every_minted_entity_id_lands_in_metadata_row.py` parametrizes over every family with an entity-creating mapper and asserts the emission contract — a future family that mints UUIDs but skips this step fails CI rather than silently importing every company with `eiti_id_company=''` (the empty-business-key SCD2 collision shape).
