...
**Technical detail:** `POST /sessions/confirm` with every member's session_id in `session_ids` (and the operator's audit stamp in the body) delegates to `BatchManager.confirm_sessions` in `packages/shared/src/shared/session/batch_manager.py`. The gate set `_DECIDED_STATES` is `TERMINAL_STATES | {CONFIRMING, CONFIRMED, IMPORTED, EXPIRED, SUBMISSION_DELETED, STALE}`. The atomic write only transitions sessions whose latest state is `CONFIRMING` — already-decided sessions come back in the response's `already_terminal` list. The pre-flight failure raises `BatchHasPendingMembers`, which the endpoint translates into a 409 response with the pending list.
