...
**Technical detail:** The dashboard status calculation lives in `apps/web_ui/components/dashboard.js` (`updateStatusBanner`): `status = sourceOnly.length > 0 ? 'BLOCKED' : ((hasErrors || hasCrosscheckIssues) ? 'NEEDS_REVIEW' : 'SUCCESS')`. The matching server-side gate is in `apps/api/src/api/session_endpoints.py` on the `/review` endpoint, which returns HTTP 422 if any validation finding lacks both a cleaner proposal and a user-supplied `USER_CHOICE` correction.
