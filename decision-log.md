...
**Technical detail:** The precedence is implemented in the mapper service's correction-collection loop at `packages/mapper/src/mapper/mapper_service.py` (`MapperService.run`): findings whose code is `FeedbackCode.USER_CHOICE` or a member of `correction_gate.FIXING_CLEANING_CODES` and that carry a `proposed_value` populate one override map, last match per coordinate winning — and because user corrections are appended to the findings list after the cleaner's, pipeline order itself yields user-over-cleaner precedence. Findings with no `(table_name, table_row_index, field_name)` triple are ignored — the override map only applies to coordinate-anchored corrections. `PLACEHOLDER_REMOVED` cells go into a separate skip set and never reach the importer. One exception sits above the whole chain: scalar entity-name cells resolved by enrichment write the registry-canonical entity name regardless of overrides, because the clean layer joins ledger to metadata by exact name equality (see the cell-value precedence list in `docs/concepts/mapper.md`).
