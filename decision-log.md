...
**Technical detail:** The importer reads the declaration UUID from the `CELL_MAPPED` finding on the synthetic `about` table (`table_name == "about"`, `field_name == "declaration_uuid"`). If that finding is missing, it returns a single `MAPPING` finding with code `DECLARATION_UUID_MISSING` and writes nothing; the session lands in `ERROR_DATA`. A genuine database write failure further down (constraint violation, connection drop) is caught separately and produces a `DB_WRITE_FAILED` finding with the underlying exception text. Logic in `ImporterService.run` and `_get_metadata_value` in `packages/importer/src/importer/import_service.py`. The UUID is a `uuid5(DECLARATION_NAMESPACE, f"{country_iso3}:{year}")` produced by the detector and surfaced as a `CELL_MAPPED` finding by the mapper.
...
## A generically-named commodity resolves to the HS "not elsewhere specified" bucket; a truly unspecified one still blocks
...
**Decision:** A generic *mineral/ore/fuel* label resolves to the HS heading that classifies exactly that unspecified case: "Minerals" / "Rocks" → 2530 (*Mineral substances not elsewhere specified*), "Metallic ores" → 2617 (*Other ores and concentrates*), "Oil products" → 2710 (*Petroleum oils, excluding crude*). The file then imports with the commodity honestly recorded as the unspecified bucket. A label that carries no usable commodity meaning — bare "Other" / "Others" (also a sector, agency-type, and project-phase value), non-mineral generics like "Grains", rock "samples", and any un-catalogued name — is **not** aliased to a bucket, but under archive-upload mode it no longer blocks either: the commodity is marked *Not available* and the file imports. The row's production value survives in the raw/resolved tiers and only the clean projection drops the now-unattributed row (a commodity row with no HS carries no clean-tier identity). Outside archive mode, an unresolvable commodity still blocks for operator review.

**Rationale:** Two different situations need two different answers. When the source named a *category* HS itself defines — "Metallic ores", "Minerals" — recording it in the corresponding *not-elsewhere-specified* heading loses no fidelity the data ever had, and lands the value in clean data attributed to that bucket. When the label carries no usable commodity meaning at all, inventing an "other commodity" dimension for it would be a fake bucket that pollutes the commodity catalog — but blocking the whole file over it (in an archive backfill with no operator present) throws away every other row too. Marking it *Not available* threads the needle: the file imports, the row's figure is preserved where it can live (raw/resolved), and no meaningless commodity entity is minted. The distinction from a *drop* matters — dropping the row would delete its production value before it ever reached a tier; NV keeps it.

**Technical detail:** The generic *mineral* terms are aliases in `COMMODITY_TRANSLATIONS` (`packages/stores/eiti/src/eiti/sdf_vocabulary.py`) on the `2530` / `2617` / `2710` canonicals, resolved by the cleaner's `resolve_commodity_alias` arm; bare `Other` is deliberately excluded from the `2617` alias set (documented inline) because it collides with the `Other` canonical key in `SECTOR_TRANSLATIONS`, `AGENCY_TYPE_TRANSLATIONS`, and `PROJECT_PHASE_TRANSLATIONS`, and the `fold_commodity_aliases` ambiguity rule would drop a form resolving to more than one canonical anyway. Everything the alias arm cannot attribute is then caught by `ArchiveUnresolvableCommodityToNotAvailableRule` (`packages/cleaner/src/cleaner/rules.py`), gated on `accept_legacy_archive_imports` and run immediately after the alias rule, which proposes `NotAvailable.NV` for the remaining `COMMODITY_UNKNOWN` cells and logs `CleaningCode.COMMODITY_MARKED_NOT_AVAILABLE` (a fixing code, so the review gate counts the cell covered). `test_commodity_recognition.py`'s alias round-trip pins each generic term to its heading; `test_unresolvable_commodity_to_nv_rule.py` pins the archive-mode fallback.
