...
**Technical detail:** Both sides go through `match_key` (`packages/shared/src/shared/aliases.py`): Python `str.strip()` (Unicode whitespace), NFC normalisation, `casefold()`. The same function is registered as a SQL function and used by the clean-tier GFS derivation, so "the crosscheck accepts this stream name" and "the import can match it" agree by construction. Note that this is the consistency-check rule and is separate from the enricher's entity-resolution normalisation, which also handles Unicode transliteration.
