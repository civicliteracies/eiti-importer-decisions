...
**Situation:** On v2.0 and v2.1 company-revenue rows (the company-payments table on the Part 5 sheet), the Sector column is filled by an Excel VLOOKUP formula rather than typed by the country submitter. The formula in the template pulls the wrong column from the Companies sheet: it returns "Company type" values (State-owned enterprise, Publicly listed company, and so on) instead of the actual sector values (Oil, Gas, Mining, Oil & Gas, Other). The bug is in the EITI template itself, not in the importer. Real files show the inconsistency in different ways: v2.0 Norway shows company registration numbers in the Sector column, v2.0 Philippines happens to land on correct sectors, and v2.1 files generally show company-type strings.

**Question:** Is the template formula wrong (and should it point at the Sector column), or was the column intentionally repurposed to carry Company type? The right long-term fix depends on the answer: if the formula is wrong, EITI ships a corrected template and the importer starts enforcing the list of valid sectors; if the column was intentionally redirected, the field is renamed from "Sector" to "Company type" on our side.

**Status quo:** Until EITI clarifies, the importer accepts whatever value lands in the Sector column on v2.x company-revenue rows without checking it against the list of valid sectors. The dashboard will display whatever the cell contains. Values landing in the database are therefore unreliable — any row may carry a company-type string, a registration number, or a real sector — and any dashboard that aggregates by Sector produces meaningless buckets. Test-deploy release notes call this out. Enforcing the sector list now would block essentially every v2.x file even though the bad data is the template's fault, not the submitter's, so keeping the field loose is the lesser harm while EITI decides.

**Technical detail:** The v2.1 template formula in question is `=VLOOKUP(C15, Companies[], 3, FALSE)` — column index 3 returns "Company type" rather than column index 5 ("Sector"). The field is currently declared `sector: str | NotAvailable` in `packages/parser/src/parser/domain/schemas/v2p1.py` on `CompanyRevenueRow` and in `packages/parser/src/parser/domain/schemas/v2p0.py` on `CompanyRevenueRowV2P0`, each with a comment that enum validation is suspended until EITI confirms. Note that the v2.0 file's internal Excel table name for this data is `Table10` (renamed to `Gov_revs_comp_proj` in v2.1) — both surface as the Part 5 - Company data sheet to the user. When EITI resolves the ambiguity, the field type should be lifted to `Sector | NotAvailable`, the suspension comment dropped, an enum-membership assertion added to `tests/unit/test_v2p1_validators.py`, and existing v2.x rows audited for backfill.
...
**Technical detail:** The dashboard status calculation lives in `apps/web_ui/components/dashboard.js` (`updateStatusBanner`): `status = sourceOnly.length > 0 ? 'BLOCKED' : ((hasErrors || hasCrosscheckIssues) ? 'NEEDS_REVIEW' : 'SUCCESS')`. The matching server-side gate is in `apps/api/src/api/endpoints.py` on the `/review` endpoint, which returns HTTP 422 if any validation finding lacks both a cleaner proposal and a user-supplied `USER_CHOICE` correction.
...
**Technical detail:** Implemented in `packages/importer/src/importer/import_service.py` step 6 (ledger `DELETE FROM <ledger_table> WHERE eiti_id_declaration = :uuid` against every registered ledger table) and step 8 (`_generate_clean_tables`, which re-runs the submission's `clean_queries` `INSERT...SELECT` statements). The clean tables don't need an explicit delete — the `INSERT...SELECT` queries are written to overwrite per-declaration data when the ledger is replaced.
...
**Technical detail:** Endpoint `delete_declaration` in `apps/api/src/api/endpoints.py` requires a `DeleteRequest` body (`full_name`, `email`, `role`, `channel`). The browser-side flow issues a `confirm()` dialog before firing the `DELETE /declarations/{declaration_uuid}` request. The endpoint constructs an `AuditStamp` and calls `TargetDbManager.delete`.
...
**Technical detail:** Layers 1 and 3 use SHA-256 over the upload bytes, indexed on `metadata_import_events.file_sha256`. Layer 1: `POST /uploads` calls `TargetDbManager.find_active_import_by_hash`, which queries `metadata_import_events` for a `file_import` event with `status = success` whose linked `metadata_summary_data_files` row is not soft-deleted; a match returns 409 with the prior `source_identifier`, `event_timestamp`, and `eiti_id_declaration`. Layer 2: `DetectorService` runs the submission's `cohort_schema.extractor` over the workbook, emits one `COHORT_DETECTED` finding per cohort, then calls `TargetDbManager.exists` on each cohort's `existence_key`; cohorts are tagged `COHORT_DUPE` or `COHORT_NEW`; all-DUPE adds a terminal `DUPLICATE_SUBMISSION` finding and pushes the session to `ERROR_DATA`, any other mix routes to the `SELECTION_CONFIRMING` interrupt. Layer 3: `POST /sessions/{id}/confirmation` re-runs `find_active_import_by_hash` and also calls `EventManager.find_active_sessions_by_hash` to detect in-flight peer sessions whose latest state is not in `DEDUP_INACTIVE_STATES`.
...
**Technical detail:** Gated by `Settings.dedup_uploads_by_hash` (`bool | None = None`, profile-driven). The LOCAL profile sets it to `False`; the DEV, TEST, STAGING, and PROD profiles set it to `True`. Both the upload-time check and the confirmation-time check guard their body with `if settings.dedup_uploads_by_hash:` — when False, both branches no-op and no SHA-256 dedup runs at either point. There is no per-request bypass. Field defined in `packages/shared/src/shared/settings.py`. Consumed at `endpoints.upload` and `endpoints.confirmation_post` in `apps/api/src/api/endpoints.py`.
...
**Technical detail:** At `POST /sessions/{id}/confirmation`, after the committed-import check, the endpoint calls `EventManager.find_active_sessions_by_hash` to find every other session whose `UPLOADED` event carries the same SHA-256 and whose latest state is not in `DEDUP_INACTIVE_STATES` (terminal + IMPORTED + EXPIRED + SUBMISSION_DELETED + STALE). If any are found, the 409 body carries `sibling_session_ids`, `sibling_batch_ids`, and a `release_actions` list — one entry per sibling session pointing at `POST /sessions/{sid}/kill`, plus one per sibling batch pointing at `POST /batches/{bid}/kill`. Sibling lookup in `EventManager.find_active_sessions_by_hash` at `packages/shared/src/shared/session/event_manager.py`. Response shape assembled in `confirmation_post` at `apps/api/src/api/endpoints.py`. The kill endpoints write a `CANCELLED` event and delete the cached `PipelineContext`, which releases the hash slot immediately. The CLI surfaces the choice via a `questionary` prompt.
...
**Technical detail:** In `packages/shared/src/shared/stats_config.json`, `currency_field.summary_v1` is `{"gov": null, "comp": null}` for both sides. `_check_currency_consistency` in `packages/crosschecker/src/crosschecker/crosschecker_service.py` short-circuits when both fields are null. The v1 totals spec omits `group_by`, producing a scalar comparison.
