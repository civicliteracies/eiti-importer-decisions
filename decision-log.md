...
**Status quo:** `FIELD_MULTI_VALUE` parser code fires only when the cell contains ≥2 *canonical* enum values separated by `[,;]`. `"Oil, Gas, Condensates"` does not match canonical tokens and falls through to `DATATYPE_INVALID`, surfacing in the review tab so the operator sees it. Operators reviewing Liberia-shaped files must reject the row and ask the submitter to split it, or pick a single canonical commodity from the dropdown.

**Technical detail:** The compound-detection logic is `detect_compound_commodity` in `packages/stores/eiti/src/eiti/parsing/schemas/validation_helpers.py`. When resolved: either extend `Commodity` enum members with aggregates, or update user-facing guidance on multi-commodity reporting and keep current behaviour.
...
**Status quo:** The behaviour splits by upload mode. On a **live** submission `FIELD_MULTI_VALUE` emits a source-only finding when ≥2 canonical commodities (or sectors) are detected in one cell: the operator must split the row in Excel, the cleaner does not auto-split, and the file cannot import until they do — the pipeline never silently flattens or duplicates. On a **legacy archive** import (`accept_legacy_archive_imports`), the parser auto-splits the compound cell into one row per canonical value (fan-out), because the historical file cannot be re-requested. The general EITI question — is compound encoding valid syntax or a reporting error? — is still open; the archive auto-split is a pragmatic bulk-import decision, not a ruling that compound encoding is valid.

**Technical detail:** Detection is `detect_compound_commodity` / `detect_compound_sector` in `packages/stores/eiti/src/eiti/parsing/schemas/validation_helpers.py`; the live source-only block routes through `FIELD_MULTI_VALUE` in `_map_pydantic_errors` (`packages/parser/src/parser/validation/row_validator.py`). The archive auto-split is the fan-out transforms over the shared `fan_out_compound_rows` core (`packages/stores/eiti/src/eiti/parsing/transforms/`), archive-gated in that core; a fan-out renumbers rows, so `parser.transforms.row_identity` re-anchors each finding's coordinate to its row's final position. When EITI resolves the general question: either confirm row-split is the canonical live workflow (no live change) or extend the auto-split to live mode with the downstream changes it implies (mapper row enumeration, clean-table aggregation semantics).
...
**Technical detail:** The bucket is a projection of the finding code's `resolution_policy` (part of its `Disposition`). A per-cell validation code's policy is shape-derived: a finding with `candidates` populated and no cleaner `proposed_value` is "needs-choice"; a finding with `proposed_value` set is "auto-fixed"; a finding with neither is "source-only". A code with a fixed policy (an enricher source outage, a crosscheck data gap) carries its mode directly. The derivation lives on the Finding model in `packages/core/src/core/diagnostics.py` as `resolution_mode`, set by a Pydantic `model_validator(mode='after')` that dispatches on the code's `resolution_policy`, so the value travels across the wire alongside the raw fields. Both the client gate (`isBlocked` in `apps/web_ui/blocked-status.js`) and the server gate (`validate_corrections_cover_fixable` in `packages/core/src/core/correction_gate.py`) read `resolution_mode` directly — a single source of truth for *which mode a finding carries*. The `resolution_mode` axis was never the drift point; the per-cell **coverage** verdict was — each display lens re-derived "is this cell covered?" with its own predicate, so a cell could read green while the gate still blocked. That verdict is now itself single-sourced by `core.findings.compose_findings` (ADR-057): the gate, mapper, CLI, and web lenses all project the one composed record instead of re-deriving coverage. The dashboard cards read the same field via `classifyFinding(f)` in `apps/web_ui/dashboard-utils.js`.
...
**Technical detail:** Whether a finding blocks is carried by its `resolution_mode`, independent of its `FindingCategory` — the gate blocks on any finding whose mode is in `core.findings.coverage.BLOCKING_RESOLUTION_MODES` = `{NEEDS_CHOICE, SOURCE_ONLY, SYSTEM_UNAVAILABLE}`. The dashboard status calculation lives in `apps/web_ui/components/dashboard.js` (`updateStatusBanner`), reading the server's composed per-cell verdict (`context.composed_findings`, ADR-057) via `summarizeComposedFindings`: BLOCKED when any cell is `blocked` (a source-fix block) or any coordless `session_blocks` entry is `source_only`/`system_unavailable`, else NEEDS_REVIEW when needs-choice cells or crosscheck issues remain, else SUCCESS — so a covered source-only cell no longer reads BLOCKED, and a coordless structural block (a missing sheet) is named as a source-fix, not an outage. The matching server-side gate lives in `packages/core/src/core/correction_gate.py` (`validate_corrections_cover_fixable`), called by `BatchManager.bulk_review` via `POST /sessions/review`; the bulk endpoint returns HTTP 422 with `BulkReviewConflictsResponse` listing each session whose uncovered blocking findings remain. The undeclared-entity gap is a `CROSSCHECK`-category finding carrying `resolution_mode=SOURCE_ONLY`; the dead-source block is an `ENRICHMENT`-category finding carrying `resolution_mode=SYSTEM_UNAVAILABLE`.
...
### What decides which review lane a flagged problem lands in, and whether it blocks?
<!-- scenario: trust-the-data; topic: data-quality-policy -->

**Situation:** A flagged problem can appear in several places — a fix dropdown, the Auto-cleaned list, the Cross-table Checks banner, the Schema Deviations section — or nowhere the operator sees. It can also block import or not. The operator needs the same kind of problem to behave the same way every time, and to behave the same way in the dashboard, the command-line tool, and any export.

**Decision:** How the tool treats a flagged problem is fixed by the **kind of problem** (its finding code), not by which check happened to raise it or which screen renders it. Each kind declares, in one place: which review lane it appears in, whether a human reviewer sees it at all (surfaced as work, surfaced read-only, or recorded only to the logs and the database), and whether it blocks import. The same kind of problem therefore lands in the same lane and blocks-or-not identically across the dashboard, the CLI, and exports — there is no screen where a given problem quietly behaves differently.

One case this makes explicit is an entity named on a payment or revenue row but not found among the report's declared entities. If the report carries a roster for that entity type and the entity is genuinely absent from it, that is a blocking data gap (BLOCKED — the operator declares it in the source and re-uploads). If the report carries no roster for that entity type at all (for example a company that appears only as a project affiliate, where the report declares no project roster), the tool cannot judge the entity missing, so it surfaces the reference as a read-only cross-table observation and does not block. The two outcomes are distinct, separately labelled findings.

**Rationale:** Tying treatment to the kind of problem removes the class of surprise where "the same warning blocked last week's file but not this one," which happens when treatment is decided per-check or per-screen. It also forces a deliberate choice for every kind of problem — a new kind cannot ship without stating its lane, its reviewer visibility, and whether it blocks — so nothing falls through to a default the operator would have to reverse-engineer. Splitting the unregistered-entity case into a blocking form and an observation-only form keeps the block honest: the tool blocks when the operator can act (declare the missing entity), and merely records when it cannot judge the reference against any roster.

**Technical detail:** Every finding code carries a `Disposition` (category, terminal, resolution policy, reviewer visibility, correction mode) as an intrinsic property, and a finding's `category` and `resolution_mode` are derived from it; see [ADR-056](../adr/056-finding-disposition-intrinsic-to-code.md) and [core.md](../concepts/core.md#finding-model). The unregistered-entity split is `CrosscheckCode.COMPANY_UNREGISTERED` / `AGENCY_UNREGISTERED` / `PROJECT_UNREGISTERED` (blocking, `SOURCE_ONLY`) versus the `*_UNREGISTERED_NO_ROSTER` observation variants (informational) in `packages/core/src/core/diagnostics.py`. An unreadable file caught at the identification stage is its own terminal code, `DetectionCode.SOURCE_FILE_UNREADABLE`, distinct from a file that opens but fails to parse (`ParserCode.FILE_OPEN_FAILED`); both end the session.
...
**Technical detail:** Computed by `computeCoverageHeadline({ met, notMet, partial })` in `apps/web_ui/dashboard-utils.js`, called from the coverage card render in `apps/web_ui/components/dashboard.js`. The function returns `{ applicable, pct }` or `null` (when applicable is zero). Bucket counts come from the coverage measure (`eiti.measures.coverage`), computed server-side over the review-time overlay: it walks the per-year `assessment_data_{year}` tables and classifies each expectation cell against the canonical Met/Not Met/Partially Met/N/A lists declared in `CoverageStatsConfig`, and the dashboard renders the resulting counts.
...
**Technical detail:** Detection lives in `packages/core/src/core/parsing/validation.py` in `validate_template_values` (the generic mechanism — angle-bracket placeholder + phrase-set membership); the EITI instruction phrases are store-declared in `packages/stores/eiti/src/eiti/parsing/schemas/template_instructions.py` and seeded into the validator's registry at `build_registries`. The validator rejects both shapes as `DATATYPE_INVALID` errors whose message carries the marker `"template placeholder"` or `"template instruction"`. The cleaner picks those up via `PlaceholderRemovalRule` in `packages/cleaner/src/cleaner/rules.py`, which keys on those marker substrings and emits a `PLACEHOLDER_REMOVED` cleaning finding whose effect is to drop the cell.
...
### When two automatic fixes land on the same cell, which one wins?
<!-- scenario: fix-problems-before-import; topic: data-quality-policy -->

**Situation:** More than one of the tool's automatic corrections can target the same cell. A rule that recognises a misspelled enum value proposes the corrected value; a legacy-archive fallback rule proposes "Not available" for a cell the operator left in a state no real value could be read from. When both fire on one cell they propose different values for the same coordinate.

**Decision:** A repair always beats a fallback fill. The tool ranks the competing fixes by a fixed priority: any rule that recovers or standardises a genuine value — an enum correction, an alias resolution, a sentinel standardisation, a URL/year/HQ-country repair — outranks any rule that substitutes a placeholder because nothing usable was supplied ("Not available" / "Not applicable" fallback fills, the anonymous-entity placeholder, the give-up "commodity marked not available"). The repair's value lands in the database; the fill is discarded. A reviewer's own choice and a corrections-manifest normalisation still sit above every automatic fix.

**Rationale:** The winner used to depend on which cleaning rule happened to run first. If the fallback fill ran after the repair, the cell would silently import "Not available" and throw away the value the tool had recovered — the same order-dependence fragility the curated-alias-vs-fuzzy rule avoids on the emit side. Fixing the priority makes the outcome deterministic and matches how a reviewer reasons: a recovered real value is always better than a "we couldn't read this" placeholder (BUG-060/062).

**Technical detail:** `resolve_overlay` in `packages/core/src/core/findings/overlay.py` folds a coordinate's competing findings by the total order `(tier, ordinal)` and takes the maximum, independent of the order findings were emitted. Every `FIXING_CLEANING_CODES` member shares one tier, so `_FIXING_ORDINAL` — a curated tuple listing all fallback-fill codes below all repair codes — breaks the within-tier tie. An import-time check bonds `_FIXING_ORDINAL` to `FIXING_CLEANING_CODES` (every code present exactly once, no duplicates). The ordinal only ever discriminates within the fixing tier: a manifest normalisation (tier 4) or a user choice (tier 5) still wins outright, and the cross-axis cell-vs-token arbitration in `read_cell` keeps comparing tier alone (BUG-061).
...
**Decision:** The Companies-sheet `commodities` field is typed as a list and validated per-element. The parser segments the cell on `,;`, ` and `, ` et `, ` y `, `&`, `/` and validates each token against the `Commodity` enum, emitting one finding per misspelled or unknown token. Resolved tokens accumulate into the `clean_company_commodities` junction table downstream. The Projects-sheet `commodities` field stays single-value. When two or more canonical commodities are detected in a single Projects cell, the tool emits `FIELD_MULTI_VALUE` and the operator must split the source row before the file can import. Compound detection is alias-aware: `"Iron Ore, Gold Ore"` resolves both tokens via aliases and triggers the same code as `"Iron (2601), Gold (7108)"`.
...
**Technical detail:** `segment_commodity_list` runs as the BeforeValidator on the Companies `commodities` field (`packages/stores/eiti/src/eiti/parsing/schemas/v2p0.py` and `v2p1.py`). It NFKC-normalises, normalises spacing around `(`, protects canonical values that themselves contain a separator (e.g. the Niobium 2615 heading) so the splitter does not shred them, then splits on the separator class. Sentinel cells (`"Not applicable"`, `"Not available"` and their accepted variants) pass through unchanged so Pydantic's `NotApplicable` / `NotAvailable` union arm matches. `detect_compound_commodity` lives in the same module and routes through `FIELD_MULTI_VALUE` via `_map_pydantic_errors` in `packages/parser/src/parser/validation/row_validator.py`. Per-token findings carry a `token_index: int | None` field on the Finding model so the mapper override key and the per-cell review UI can address one token at a time.
...
**Technical detail:** `ListTokenNoiseFilterRule` in `packages/cleaner/src/cleaner/rules.py` consumes per-token `DATATYPE_INVALID` findings (where `token_index is not None`) and emits `CleaningCode.NOISE_TOKEN_REMOVED`. It runs BEFORE `AliasResolutionRule` so noise tokens don't trip alias or fuzzy correction first. Whole-cell sentinels short-circuit inside `segment_commodity_list` in `packages/stores/eiti/src/eiti/parsing/schemas/validation_helpers.py` and `segment_company_list` next to it.

The coverage classifier in `packages/core/src/core/findings/coverage.py::coverage_class` (folded per cell by `core.findings.compose_findings`, ADR-057) distinguishes the dropping intent of `NOISE_TOKEN_REMOVED` from the repairing intent of fixing cleanings (`EN_ALIAS_RESOLVED`, `ENUM_CORRECTED`, etc.). A `NOISE_TOKEN_REMOVED` at token N covers ONLY token N — it does NOT cover cell-level validations or sibling-token validations on the same cell. A fixing cleaning at token N covers same-coord validation AND any cell-level DATATYPE_INVALID that Pydantic emitted when the bad token first failed a `list[T] | Sentinel` union arm. The distinction matters for cells with mixed shapes (one bad token plus noise on the same cell): the noise removal does not paper over the bad token, and the operator still sees an actionable review prompt.
...
**Technical detail:** The auto-fill rule is `ArchiveMapToNotAvailableRule` in `packages/cleaner/src/cleaner/rules.py`; it fires only on `ParserCode.CELL_BLANK`, which is itself only emitted when the field's type union explicitly contains `NotAvailable` (see `blank_cell_code_for` in `packages/parser/src/parser/validation/row_validator.py`). The free-text carve-out is the `Annotated[str | Blank, BeforeValidator(_none_to_blank)]` alias in `packages/stores/eiti/src/eiti/parsing/schemas/validation_helpers.py`.
...
**Technical detail:** The three parser codes are `CELL_BLANK` (non-blocking, auto-fills via `ArchiveMapToNotAvailableRule`), `CELL_BLANK_BLOCKING` (no sentinel legal), and `CELL_BLANK_DEPENDENT` (`NotApplicable` legal but not `NotAvailable`). The code is derived by `blank_cell_code_for(model, field_name)` in `packages/parser/src/parser/validation/row_validator.py`, which calls `get_args` on the field's annotation and returns `CELL_BLANK` if `NotAvailable` is in the union, `CELL_BLANK_DEPENDENT` if `NotApplicable` is in it, and `CELL_BLANK_BLOCKING` otherwise. The dependent-case candidate is pulled by `_extract_enum_candidates` from the Pydantic enum error on the union.
...
**Technical detail:** The dispatch lives in `_map_pydantic_errors` in `packages/parser/src/parser/validation/row_validator.py`: blank cells route to `blank_cell_code_for(...)`; cells where the BeforeValidator raised `CompoundValueError` route to `FIELD_MULTI_VALUE`; cells whose text is a wrong-sentinel variant on the field's accepted set route to `SENTINEL_WRONG` (with the accepted sentinel injected into `candidates` for the cleaner); everything else falls through to `DATATYPE_INVALID`. Whitespace-to-`None` normalisation happens in `validate_template_values` in `packages/core/src/core/parsing/validation.py`. The cleaner repair rules — `PlaceholderRemovalRule`, `StandardizeNotAvailableRule`, `StandardizeNotApplicableRule`, `WrongSentinelCorrectionRule` (cross-sentinel NA↔NV correction), three per-language `AliasResolutionRule` instances (deterministic EN/FR/ES alias resolution against the lookups in `packages/stores/eiti/src/eiti/sdf_vocabulary.py`), `EnumCorrectionRule`, and `ArchiveMapToNotAvailableRule` — live in `packages/cleaner/src/cleaner/rules.py` and run in that order so deterministic standardisation precedes fuzzy fallback.
...
**Technical detail:** The row-aware logic lives in `@model_validator(mode="before")` methods on the row models — see `cascade` on `CompanyRevenueRowV2P1` in `packages/stores/eiti/src/eiti/parsing/schemas/v2p1.py` and the shared helper `cascade_metadata_row_na` in `packages/stores/eiti/src/eiti/parsing/schemas/validation_helpers.py`. The "Not available" auto-fill is `ArchiveMapToNotAvailableRule` in `packages/cleaner/src/cleaner/rules.py`, gated on `f.code == ParserCode.CELL_BLANK` — it never matches `CELL_BLANK_DEPENDENT` or `CELL_BLANK_BLOCKING`.
...
**Technical detail:** The three header-search schemas in `packages/stores/eiti/src/eiti/parsing/schemas/api_extract_v1.py` each declare a `row_filter` callable that inspects each row's `organisation.type` cell. The agency schema admits rows whose value is `"agency"`; the company-revenue and company-metadata schemas admit rows whose value is `"company"`. Everything else is rejected before the row reaches Pydantic validation, so no findings are produced for the dropped rows.
...
**Technical detail:** The v2.0 and v2.1 schemas in `packages/stores/eiti/src/eiti/parsing/schemas/v2p0.py` and `packages/stores/eiti/src/eiti/parsing/schemas/v2p1.py` map the header `("Exchange rate used: 1 USD =", 2)` to the field `exchange_rate_used`. The conversion direction is implemented once in `convert()` in `packages/stores/eiti/src/eiti/currency_conversion.py`: a local-currency row produces `val / exchange_rate` for the USD side, a USD row produces `val * exchange_rate` for the local side. Both the mapper's assembly seam (when it builds resolved rows) and the pre-import revenue measure call `convert()`, so the stored `*_usd` columns and the pre-import preview use one implementation.
...
**Technical detail:** `sanitize_rate` in `packages/stores/eiti/src/eiti/currency_conversion.py` rejects zero, non-finite, and non-positive values at the data-ingestion seam; the parser seam and the revenue measure call it on the About-sheet rate before assigning `exchange_rate`. Because `exchange_rate` stays `None`, the v1 fallback block immediately below runs unchanged for v1 submissions.
...
**Decision:** A single `sanitize_rate` function at the data-ingestion seam (`eiti.currency_conversion.sanitize_rate`) rejects 0, non-finite (±Inf, NaN), and non-positive values; it returns `SanitizedRate | None` so the type system propagates the absence of a rate from there forward. The parser's Pydantic schemas wrap the rate field with a `BeforeValidator` calling the sanitizer, and the revenue measure calls it on the About-sheet cell + the v1 archive entry before passing to the universal rule. Downstream consumers therefore see only positive finite rates or None.
...
**Technical detail:** `sanitize_rate` in `packages/stores/eiti/src/eiti/currency_conversion.py` applies the predicate: reject if the value is `None`, not finite (`+Inf`, `-Inf`, `NaN`), or non-positive. The parser schemas wrap the rate field with `BeforeValidator(sanitize_rate_cell)` so unsanitised rates cannot even land on a parsed row; the revenue measure calls the function on About-sheet and v1-archive rates before any consumer uses them.
...
**Technical detail:** The dual totals are the revenue measure's ids `total_gov_revenue_local`/`_usd` and `total_company_payments_local`/`_usd` (with `reconciliation_gap_pct` as the currency-agnostic ratio), computed server-side by `eiti.measures.revenue`. The dashboard rendering rule lives in `renderFinancialValue` in `apps/web_ui/components/dashboard.js`: it renders USD primary plus a local secondary line when `mainCurrency !== 'USD' && mainCurrency !== 'Unknown'`, falls back to local-only when USD is null but local is computable, and emits an N/A card with a contextual notice when neither is available.
...
**Technical detail:** `_sum_side` in the revenue measure (`eiti.measures.revenue`) folds each table's rows via `fold_sum` (`core.aggregation`), calling `eiti.currency_conversion.convert`, sums the rows that returned a non-None side, and counts the rows that did not into `unresolvable_*_local_count` / `unresolvable_*_usd_count` (the BUG-029 per-side counts). The dashboard's `renderPartialBadge` in `apps/web_ui/components/dashboard.js` emits the `card-notice` "Partial: N rows unresolvable" when the USD-side count is positive. Post-import, the clean-SQL `build_currency_resolution_sql` builder writes one `clean_currency_resolution` row per NULL-USD clean row carrying the matching `UnresolvableReason` value.

### How does the tool validate a v2.x totals block against the data?

<!-- scenario: reconcile-government-vs-companies; topic: currency-financial-calculations -->

**Situation:** An EITI v2.x totals block carries up to two rows — "Total in [reporting currency]" and a courtesy "Total in USD". Each is a *grand-total restatement* of the whole government (or company) revenue in that currency. The data rows themselves may be split across currencies: a file reporting in MZN can carry MZN rows and USD rows, and its "Total in MZN" equals the MZN rows plus the USD rows converted at the file's rate.

**Decision:** The crosschecker validates each totals row against the data **grand total expressed in that row's currency**. It sums the data rows into per-currency buckets (a blank currency cell canonicalises to the reporting currency, matching the clean projection), then for a "Total in C" row computes the grand total in C: buckets already in C are added raw, and every other bucket is converted into C through the single About-sheet reporting↔USD rate. The reporting-currency total is the *primary* total (`GOV_TOTAL_MISMATCH` / `COMP_TOTAL_MISMATCH` on a mismatch); any other currency is a *courtesy* total (`*_COURTESY_TOTAL_MISMATCH`) — the split keeps the operator's remediation target the right cell. Comparison uses a two-tier tolerance (absolute floor of 1 currency unit; relative band of 0.1%, whichever is larger) in the row's own currency. When the grand total needs a conversion the single rate cannot bridge — the rate is missing, or a data row is in a currency that is neither the reporting currency nor USD — the total is not verifiable and the check emits the informational `*_TOTAL_NOT_VALIDATABLE` (primary) or `*_COURTESY_NOT_VALIDATABLE` (courtesy) finding rather than a mismatch. When the reporting currency itself is absent, `REPORTING_CURRENCY_MISSING` already covers the state and the totals check stays silent — one finding code per state.

**Rationale:** The naive alternative buckets the data by currency and compares each totals row to its *same-currency* bucket. That assumes the data is denominated in exactly one currency (the reporting currency), which is false two ways: a file whose data is entirely in USD while reporting in IQD has an empty IQD bucket, so its "Total in IQD" is compared to zero and false-fires "computed sum is 0.00"; and a genuinely multi-currency file has its "Total in MZN" grand total compared to only the MZN bucket, short by the USD portion. Reconciling each total against the whole-table grand total in that total's currency is the template's actual contract — every "Total in C" asserts conservation over all data expressed in C — and it validates the reporting-currency total and the courtesy USD total under one rule. Refusing (not mis-scaling) a total whose grand total needs a currency the single rate cannot reach keeps the check honest: conservation cannot be *verified* against a row the file gives no basis to convert. The two-tier tolerance clears the per-row conversion rounding a restatement accumulates while still catching material drift.

**Technical detail:** The closure in `_make_totals_consistency_check` (`packages/stores/eiti/src/eiti/families/sdf/__init__.py`) reads the About anchor once per call via `_read_reporting_anchor` (delegating to `resolve_revenue_stats`, the same `StatsCurrency` registration the revenue measure consumes) and computes each grand total via `_grand_total_in`, which converts buckets with `convert_total` only when the bucket currency and the target are both endpoints of the rate ({reporting, USD}). The spec is a discriminated union: `_GroupedTotalsSpec` (v2.x) carries the four finding codes and the relative band by construction, `_ScalarTotalsSpec` (v1) carries neither — so a grouped spec cannot omit its courtesy wiring. `test_totals_spec_variants` pins that each family registers the right variant with side-consistent codes.
...
**Technical detail:** The rule itself lives in `eiti.currency_conversion.convert` — the single source. The mapper's assembly seam calls `convert()` when it builds resolved rows (the `UsdSpec` declaration in `core/families/_mapper_protocol.py`; the calling seam is `assemble_resolved_rows` in `_resolved_assembly.py`), and the pre-import revenue measure calls it too, so stored USD values and the preview come from one implementation. `tests/unit/test_measures.py` locks the measure output and `tests/unit/test_resolved_assembly_behavior.py` locks the assembly-side rule case by case.
...
**Rationale:** Treating an empty cell as "third currency that cannot be resolved" would produce avoidable third_currency unresolvables on the great majority of well-formed files. The implicit-reporting-currency convention is how submitters and EITI reviewers already read these tables; encoding it once at the clean tier means every downstream consumer (views, the revenue measure, dashboards, API) sees the canonical value without re-implementing the convention.

**Technical detail:** The canonicalisation happens at the assembly seam: `UsdSpec` substitutes the declaration's reporting currency for an empty row currency and stores the winner in the resolved `currency_code` column, which the clean projections carry through unchanged. The revenue measure calls `_canonical_row_currency` before invoking the universal rule so the pre-import preview matches what assembly will store.
...
**Technical detail:** The fill is a parser transform (`packages/stores/eiti/src/eiti/parsing/transforms/api_extract_currency_fill.py`) registered only for the `api_extract_v1` submission, running before row validation so the "Not available" placeholder never forms. It targets the two revenue tables (`agency_revenues_api_v1`, `company_revenues_api_v1`); each fill emits a `SCHEMA_DEVIATION` finding with the dedicated parser code. Downstream, the standard USD-conversion rule treats the filled rows as USD-native.
...
**Technical detail:** In the revenue measure (`eiti.measures.revenue`), the gap is computed only when both sides on a given currency pair are non-None: `gap_local = total_gov_revenue_local - total_company_payments_local` and likewise for USD. The percentage path also guards against `base != 0.0` — a country reporting literally zero government revenue would otherwise trigger ZeroDivisionError, and a bare-truthy check would have silently dropped the percentage in that case. This is the single definition; both clients render `reconciliation_gap_pct` without recomputing.

**Note on partial-aggregate asymmetry:** Under the partial-aggregate semantics, each side's total is a sum of the rows the universal USD rule could resolve on that side; rows it could not resolve are counted into `unresolvable_*_count`. The gap can therefore compare row populations that aren't identical — one side may have excluded 5 third-currency rows the other side fully resolved. The tool does NOT null the gap when either side has unresolvable rows; the partial gap is shown alongside the per-side badge counts so the operator sees both the figure and the proportion it covers. Nulling the gap whenever either side was partial would reintroduce the all-or-nothing behaviour the partial-aggregate refactor (BUG-029) was designed to remove — an unresolvable EUR row in a 30-row company-payments table would hide the gap for the other 29 rows on both sides. The badge counts (`unresolvable_gov_*_count`, `unresolvable_comp_*_count`) on the statistics response carry the missing context.
...
**Technical detail:** In the revenue measure (`eiti.measures.revenue`): `gap = gap_local; base = total_gov_revenue_local`, then `if gap is None or base is None: gap = gap_usd; base = total_gov_revenue_usd`. The percentage is a single scalar measure `reconciliation_gap_pct` — there is no separate `_local` and `_usd` version. It is computed once, server-side, and both clients render it.
...
**Technical detail:** In the `RevenueStatsConfig` for `summary_v1` (gathered by `stats_configs()`), `StatsCurrency.gov_per_row` and `StatsCurrency.comp_per_row` are both `None`. The revenue measure reads those nulls and so never tries to look up a per-row currency on a v1 row; `_canonical_row_currency` then canonicalises the absent cell to the reporting currency, and `convert()` routes the row through its `row_currency == reporting_currency` branch. The currency-quality crosscheck in `packages/stores/eiti/src/eiti/families/sdf/__init__.py` skips v1 on the same signal — its guard returns no findings when `StatsCurrency.gov_per_row` and `comp_per_row` are both null.
...
**Decision:** The tool ships with a committed lookup table of historical exchange rates, extracted from EITI's API export and keyed by country and year. When a v1 file's About sheet has no rate, the stats computation looks up the rate by the file's country code and reporting-period end year, server-side.

**Rationale:** Historical exchange rates are stable and available from EITI's own data. A static, committed table avoids runtime dependency on external services and keeps historical USD totals reproducible across releases.

**Technical detail:** Lookup file `v1_exchange_rates.json`, keyed by `{iso3}_{year}`. The revenue measure resolves the rate using `country_iso3` + `end_date` year, server-side. The SDF mapper consults the same archive at import time (`packages/stores/eiti/src/eiti/families/sdf/__init__.py::build_sdf_metadata_rows`), so the rate that ends up in `metadata_summary_data_files.exchange_rate_used` is the same value the pre-import preview displayed.
...
### Do the pre-import figures reflect my review corrections?
<!-- scenario: reconcile-government-vs-companies; topic: currency-financial-calculations -->

**Situation:** During review an operator fixes cells — a cleaner rewrite, an enricher mint, or a manual correction. The dashboard shows the revenue totals, the reconciliation gap, and the commodities breakdown before the import commits, from that same session.

**Decision:** The pre-import revenue, gap, and commodities figures read the review-time state — the extracted data with the operator's findings applied — not the raw parsed rows. Correcting a revenue or currency cell moves the figure on the next render.

**Rationale:** An operator reviewing corrected data expects the headline figures to reflect what they just fixed; showing raw-parse totals beside corrected cells is misleading. Reading the reviewed overlay makes the preview a faithful re-lock over the reviewed submission rather than a snapshot of the raw parse. The figures reflect the review-time state, not the committed rows byte-for-byte: the mapper commits through a superset overlay after confirm, so the stored data is a superset of what the preview aggregated.
...
**Technical detail:** The classification is computed in the browser by `updateStatusBanner` in `apps/web_ui/components/dashboard.js`. It reads the server's composed per-cell verdict (`context.composed_findings`, ADR-057) through `summarizeComposedFindings` — source-fix blocks (`blocked` cells + coordless `source_only`), in-app decisions (`needs_choice`), and a genuine system-unavailable outage, each counted once. The composed field is itself derived live from the findings the pipeline produced (a `@computed_field`, recomputed per dump), so the status is never stored on the server. The same classification feeds the colored quality bands shown on the dashboard cards.
...
**Decision:** A finding is keyed by its claim — the cell it points at, the code, and what it asserts about that cell. A step that observes a *new* claim adds a finding; a step that re-observes an *existing* claim (a re-run, a resumed session, a replay) does not add a second copy — it replaces that finding's improvable payload (its message, its candidate list) with the latest derivation and keeps its place in the list. Distinct claims never merge. A user correction is a finding at a new claim (a different code), so it is added next to the original, never on top of it — and at most one correction is allowed per cell and code (a payload carrying two is rejected at the review request). The original problem the tool first detected always remains visible at confirmation time.

**Rationale:** Everything the user reviewed has to match exactly what the tool will act on at import time. Two copies of one claim are not two facts — they are one fact counted twice, which is what a re-run or replay would produce if findings only ever grew. Keying findings by claim makes that double unrepresentable, so a re-run lands the same finding rather than a duplicate, and a resumed in-flight session shows the latest derivation of each claim (an improved candidate list, say), not a stale first copy. The audit trail stays a complete account of every distinct thing each step observed and what the user said about it.

**Technical detail:** `PipelineContext.findings` is exposed as a read-only `Sequence[Finding | ParsingError | DetectionResult]` — `context.findings.append(...)` / `.pop()` are compile errors — wrapped by an `AfterValidator` into an immutable, identity-unique `FindingCollection` (`packages/core/src/core/finding_collection.py`), a dict keyed by `Finding.identity` where a re-emitted claim is absorbed (last-write-wins on the non-identity payload). Every finding's `identity` is a pure function of its inputs (see [ADR-059](../adr/059-finding-instance-identity.md)), so re-derivation is idempotent by construction. All mutation flows through `PipelineContext.add_findings(...)` (which reassigns the field, re-folding it under `validate_assignment=True`) or `merge_corrections`. User corrections come in as new findings with code `USER_CHOICE`; escalations as new findings with code `FLAGGED`; `merge_corrections` routes them through the same identity fold. The mapper's precedence rule (USER_CHOICE wins over cleaner overrides wins over extracted data) is enforced by ranking the findings on one precedence ladder in the `CellOverlay`.
...
**Technical detail:** Scoring lives in `score_templates` in `packages/parser/src/parser/identification/matcher.py`. Each `SubmissionDefinition` in `packages/stores/eiti/src/eiti/parsing/submissions.py` supplies its `FingerprintRules.version_string` regex and its `schemas` sequence; the schemas whose `used_in_identification` flag is set feed the structural signal. The three-signal weighting is 50 / 20 / 80 on a 0-150 scale. The dispatcher inside `score_templates` picks one of `check_standard_table`, `check_kvp`, or `check_fixed_columns` per schema type.
...
**Technical detail:** `SUBMISSION_UNRECOGNIZED` is registered as a terminal finding code in `packages/pipeline/src/pipeline/factory.py` (in the `terminal_codes` set alongside `SOURCE_TIER_CONFLICT` and `FILE_OPEN_FAILED`). The detector emits it from `build_detection_findings` in `packages/parser/src/parser/identification/detector_service.py` when `result.candidates` is empty.
...
**Technical detail:** `eiti.label_recognition` (`packages/stores/eiti/src/eiti/label_recognition.py`) folds a localized sheet title or structural label to its English canonical via an accent- and case-insensitive key; the aliases are declared directly in the module (originally harvested from EN + localized corpus pairs). The parser's identification and extraction locators call it so localized sheets and labels resolve without renaming the file. The cleaner's `FR_ALIAS_LOOKUP`, `ES_ALIAS_LOOKUP`, and `RU_ALIAS_LOOKUP` in `packages/stores/eiti/src/eiti/sdf_vocabulary.py` cover localized data *values* (enum dropdowns, country names). A new localized structural label belongs in `label_recognition.py`; a new localized data-value spelling belongs in the cleaner's `*_TRANSLATIONS` tables. (The offline `normalize_localized.py` harvest tool and its manifests were later removed once the aliases became self-contained.)
...
**Technical detail:** The threshold is `DEFAULT_THRESHOLD = 86` in `packages/stores/eiti/src/eiti/families/_enrichment_helpers.py` (re-exported via `eiti.families`); the comment there explains it's a noise plateau from eiti corporate suffixes. When the user picks a suggested record during review, the pick is a person-authored `feedback:entity_choice` finding carrying the chosen registry candidate; it is routed through the same entity-recognition code-sets as the enricher's own matches (`core.diagnostics.AMBIGUOUS_RESOLVING_CODES` et al.), so `emit_entity_metadata_findings` in `packages/mapper/src/mapper/mapper_service.py` skips minting a fresh id at that coordinate and the group adopts the picked entity. The skip is keyed on the finding's full coordinate (token index included), so a pick on one element of a list-entity cell resolves exactly that element. An ambiguous mention the user does not resolve falls through to the same deterministic mint as a MATCH_NONE (no match).
...
**Decision:** Two source kinds contribute entity rows, tagged into three row corpora: v1 SDF workbooks (`local/v1/`, detected `summary_v1` → `sdf_v1`), v2 SDF workbooks (`local/v2/`, `summary_v2.x` → `sdf_v2`), and the Company Assessment workbook (`local/ca/` → `ca`). The aggregate v1 API extract is **not** a dedup source — every one of its cohorts is now redundantly present as a real SDF, so it contributes no entity the SDF corpus doesn't already contribute. validation_data is excluded — its family declares no entity-creating columns, so parsing it would produce zero inventory rows. The required-files contract lives in `REQUIRED_FILES` at the top of `build_inventory.py`; missing any required corpus causes `discover_source_files()` to raise `FileNotFoundError` rather than silently absent the missing corpus from the inventory.

**Rationale:** Silent skip on a missing corpus is a lying API: the function says it discovered the source files while quietly omitting them. Fail-loud surfaces operator configuration issues at script start instead of letting them propagate into a half-empty inventory the operator debugs hours later. The `REQUIRED_FILES` declaration is the single source of truth — the architectural test (`test_arch_dedup_required_files_contract.py`) verifies that `discover_source_files()` iterates the declaration and that every `SourceKind` member has a declaration entry, so adding a new corpus is one declarative change rather than a coordinated edit across multiple sites.

**Technical detail:** Canonical paths are `local/v1/*.{xlsx,xls}` (v1 SDF corpus, one workbook per country-year report), `local/v2/*.xlsx` (v2 SDF corpus, one workbook per country-year report), and `local/ca/company_assessment_v1.xlsx` (ca workbook). The ca location matches `tools/corpus_tester/configs/ca.toml`'s `files_dir` so both the dedup pipeline and the corpus_tester read the same path; dev populates `local/ca/` by copying or symlinking from `apps/web_ui/public/examples/company_assessment_v1.xlsx`, which is the file the WebUI also serves as a sample download. The `Corpus` StrEnum in `build_inventory.py` enumerates the closed set (`SDF_V1`, `SDF_V2`, `CA`) and is enforced by the `corpus` field's enum constraint in `inventory-row.schema.json`; the `test_dedup_corpus_enum.py` test pins the two sides together so a one-sided change fails at PR time. The per-file degraded mode in `harvest_rows_from_file` (a single file failing identify/parse returns an empty row list + a status entry recorded in the run summary) is intentional and distinct from the source-file-missing case — one bad file shouldn't sink the whole inventory rebuild, but a missing required corpus should refuse to start.
...
### A re-issued permit with the same register number is one project, not several
<!-- scenario: trust-the-data; topic: entity-resolution -->

**Situation:** Some countries name a project by a state-register instrument — a permit or licence carrying a unique register number and an issue date, e.g. Ukraine's "subsoil use special permit no 1266 dated 03 10 1998". The same permit surfaces across many yearly reports, and its recorded issue date drifts: a transposed day and month (`03 10` vs `10 03`), a spacing variant, or a later renewal year on the same number. Read literally, each spelling is a different project, so one permit fragments into several entities and its disclosed money scatters.

**Decision:** For an admitted templated-permit family, a project's identity is the register number alone. Every disclosed date, spacing, and renewal-year variant of the same number collapses to one project. A code-shaped project name matching no admitted template — an opaque block or licence code that is already distinct — is left as its own project: the tool never guesses a merge there, and never sends such codes to the name-matching panel, because a merge over near-identical codes would be wrong. A free-form descriptive project name goes to the panel like any other prose entity. Today one family is admitted: Ukraine subsoil-use special permits.

**Rationale:** The register number is the state's own unique identifier for the licence, so two records sharing it are the same licence — a renewal is the continuation of one project, not a new one. Keeping the number as the identity while carrying every issue date in the record's audit trail makes the collapse faithful and reversible. A family is admitted only after a human reviews the number's collisions and confirms every one is a legitimate collapse; near-identical opaque codes are never merged automatically because a wrong merge there is unrecoverable.

**Technical detail:** The router is `tools/dedup/project_code_registry.classify` (anchored, exact template match — never a substring test); admitted templates are data in `TEMPLATE_REGISTRY`. `project_code_evidence.py` groups project rows by `(country, register-number)` and emits POSITIVE edges the resolver clusters — the identifier is the key, all issue dates ride the edge payload. A same-numbered permit re-issued in a later year merges by design; a genuinely recycled number (a bounded, accepted risk — 15 of 464 Ukraine ids show wide date spans, none carrying a differing project description) would merge silently. Measured on the baseline inventory, the lane collapses 118 Ukraine permit-name variants and perturbs no other country's project clusters.
...
**Technical detail:** The (table, field) pairs eligible for the fallback live in `FALLBACK_NV_FIELDS` in `packages/stores/eiti/src/eiti/families/sdf/archive_fields.py` (store domain data injected into the cleaner via `CleaningInputs`). Each pair's row-model schema in `packages/stores/eiti/src/eiti/parsing/schemas/v2p0.py` / `v2p1.py` carries `| NotAvailable` in its union; this widening turns what would have been `CELL_BLANK_BLOCKING` or `CELL_BLANK_DEPENDENT` into the non-blocking `CELL_BLANK` code so `ArchiveMapToNotAvailableRule` (in `packages/cleaner/src/cleaner/rules.py`) can fire on it. The rule consults `FALLBACK_NV_FIELDS` at emit time and uses `CleaningCode.NV_FALLBACK_FILL` for the audit code instead of the primary-intent `BLANK_TO_NOT_AVAILABLE`. Operator-confirmed NV (cell text "Not available" / "Non communiqué" etc.) is normalised separately by `StandardizeNotAvailableRule` and emits `NOT_AVAILABLE_STANDARDIZED` — three distinct codes for three distinct provenances.

### How does the tool handle files where the operator left identifying fields blank?
<!-- scenario: trust-the-data; topic: data-quality-policy -->

**Situation:** A subset of corpus files leave identifying entity fields blank — operator's `company_name`, `company_id`, `project_name`, or `government_entity`. Under strict validation these surface as `CELL_BLANK_BLOCKING` and the file cannot import. While the historical EITI archive is being backfilled, blocking on these breaks ~21 files even though their financial data is otherwise complete.

**Decision:** A single feature flag, `accept_legacy_archive_imports` (in `core.settings`), gates a lenient cleaner pathway. When the flag is on (opted into by the corpus tester during archive backfill), the cleaner's `ArchiveMintAnonymousEntityRule` proposes a synthetic explicit placeholder for each blank identifying cell — `"Anonymous"` for the name field, paired with a sentinel `eiti_id` of the form `eiti_<type>_00000000-0000-0000-0000-000000000000` (RFC 4122 nil UUID, same shape as regular `eiti_<type>_<uuid4>` cluster ids). The enricher resolves the synthetic name via a pre-seeded manifest alias to the sentinel eiti_id, so import succeeds with a clearly marked anonymous row. When the flag is off (the default), the cleaner does not propose, the cell stays `CELL_BLANK_BLOCKING`, and the file requires source-side correction.
...
**Technical detail:** Flag defined at `packages/core/src/core/settings.py:Settings.accept_legacy_archive_imports`. Cleaner rule at `packages/cleaner/src/cleaner/rules.py:ArchiveMintAnonymousEntityRule`. The eligible cells are `ANONYMOUS_ENTITY_FIELDS` (`packages/stores/eiti/src/eiti/families/sdf/archive_fields.py`); the rule mints on whichever blank code the field's type raises — `CELL_BLANK` for an NV-accepting identifier (the v2 fields) and `CELL_BLANK_BLOCKING` for a strict required identifier (the v1 SDF `companies_v1` / `company_revenues_v1` `company_name`, a plain `str` with no sentinel) — plus `CONTRACT_REF_IN_PROJECT_NAME`. Minting an explicit placeholder for a source-only blocking code in archive mode mirrors the disposition the rule already applies to `CONTRACT_REF_IN_PROJECT_NAME`, and is distinct from NV-filling a blocking cell (which `ArchiveMapToNotAvailableRule` refuses): the placeholder records "operator left it blank; anonymized" rather than "operator declared not available". Sentinel aliases at `packages/stores/eiti/src/eiti/resources/entity-aliases.toml` (entries with `eiti_id = "eiti_id_company:00000000-0000-0000-0000-000000000000"` etc.). Emission code is `CleaningCode.ANONYMOUS_PLACEHOLDER_FILL`. All transitional code in this scope carries the `# LEGACY_ARCHIVE_UPLOAD` marker for grep-driven cleanup.

### What happens when the About sheet's country code cell holds something that isn't a valid country code?

<!-- scenario: trust-the-data; topic: data-quality-policy -->

**Situation:** The About sheet carries an `ISO Alpha-3 Code` cell alongside the `Country or area name`. Operators sometimes enter something that is not a valid ISO-3166-1 country code there — a currency code (Chad 2019 put `XAF`, the CFA-franc code, in the country column), an ISO-2 code, or a typo. The country name in the adjacent cell is correct and unambiguous, but the invalid code fails validation and blocks the file.

**Decision:** When the `iso_code` cell is present but not a valid ISO-3 code AND the `country_name` cell resolves to a country, the tool corrects `iso_code` in place from the authoritative country name (Chad → `TCD`) and records an `ABOUT_ISO_CORRECTED_FROM_COUNTRY` finding. A blank `iso_code` is still filled from the country name (the pre-existing behavior); a *valid* `iso_code` is never overwritten even if it disagrees with the country name — a well-formed operator value is authoritative. When neither the code nor the name resolves, the code stays rejected and the cell blocks (`ABOUT_ISO_REJECTED` carries the diagnostic context).

**Rationale:** A country code that is provably not a country code, next to a country name that is unambiguous, is a certain single-cell correction — the same class as filling a blank code from the name, so it runs in every mode rather than gating on the archive flag (correcting a wrong country code is correct on a live submission too). The tool does not second-guess a well-formed code, because two valid-but-disagreeing signals are a genuine operator conflict a human should resolve, not something to auto-pick.

**Technical detail:** `derive_about_metadata_from_country` in `packages/stores/eiti/src/eiti/parsing/transforms/derive_about_metadata.py`; `_resolve_country` returns the rejected operator value whenever the typed `iso_code` fails `CountryCode(...)`, and the caller overwrites from the resolved country when one exists. Emission code `ParserCode.ABOUT_ISO_CORRECTED_FROM_COUNTRY` (SCHEMA_DEVIATION), classified `NON_VALIDATION` in `tests/unit/test_parser_code_disposition_coverage.py`.
...
**Technical detail:** Transform at `packages/stores/eiti/src/eiti/parsing/transforms/company_in_project_name_reroute.py`, registered at index 5 of `_V2_TRANSFORMS` in `packages/stores/eiti/src/eiti/parsing/submissions.py` (immediately after `drop_empty_identity_rows` — droppers run before mutators). Detection patterns: `_CHID_PATTERN` matches `\(\d{6,10}[A-Z]?\)$` (literal parens required); `_SUFFIX_PATTERN` is built from `core.legal_form_suffixes.LEGAL_FORM_SUFFIXES` (the consumer sorts longest-first at build time so multi-token suffixes match before single words). Emits `Finding(code=ParserCode.COMPANY_NAME_IN_PROJECT_NAME_REROUTED, category=FindingCategory.SCHEMA_DEVIATION)` per row modified. The detection signal (`chid` / `suffix:<word>`), the disposition (prepended / appended / duplicate dropped), and the target field (`affiliated_companies`) are described in the finding's `message` prose — the reroute carries no structured payload, as no consumer reads those facts programmatically. Classified `NON_VALIDATION` in `tests/unit/test_parser_code_disposition_coverage.py` so the operator sees an informational note but no actionable button. Architectural invariant pinned in `tests/unit/test_project_name_not_in_legacy_fallback_nv.py`: `(Projects, project_name)` and `(Companies15, project_name)` must NOT appear in `FALLBACK_NV_FIELDS` or `LEGACY_FALLBACK_NV_FIELDS`, otherwise `ArchiveMapToNotAvailableRule` would race against `ArchiveMintAnonymousEntityRule` and the row would NV-fill instead of minting the anonymous-project placeholder. Under strict mode (the default), the cleared `project_name` cell is `CELL_BLANK_BLOCKING` and the row blocks pending operator review; the corpus tester's archive opt-in mints the anonymous project instead. Flag-removal PRs must revisit the transform's downstream contract.
...
**Technical detail:** The single finding code is `REPORTING_CURRENCY_MISSING`, emitted by `_make_currency_quality_check` in `packages/stores/eiti/src/eiti/families/sdf/__init__.py` against the About table. The aggregate-tier predicate is guarded behind a presence check on the About-sheet currency value.
...
**Technical detail:** `TotalsSpec` in `packages/crosschecker/src/crosschecker/crosschecker_service.py` (tolerance defaults to 1.0); per-version configs in `packages/pipeline/src/pipeline/profiles/summary_v1.py`, `summary_v2p0.py`, `summary_v2p1.py`. Extraction schemas: `GOV_TOTALS_SCHEMA_V1` (HeaderSearch) in `packages/stores/eiti/src/eiti/parsing/schemas/v1.py`; `GOV_TOTALS_SCHEMA_V2P0` / `COMP_TOTALS_SCHEMA_V2P0` (KvpScan, pattern `^total in (.+)`) in `v2p0.py`; same shape in `v2p1.py`. Findings emit as `GOV_TOTAL_MISMATCH` / `COMP_TOTAL_MISMATCH`.
...
**Technical detail:** `is_aggregate_label()` and `_AGGREGATE_LABEL_PATTERNS` in `packages/core/src/core/entity_norm.py`. Wired into the pure identity fold's floor (`_identity_key`) alongside `is_placeholder()`; sentinel recognition itself lives in `recognize_entity_name`, not the fold. Negative-triangulation tests against real corporate names live in `tests/unit/test_entity_norm.py::TestIsAggregateLabel`. Distinct from the in-file numeric-total crosscheck above: that compares values across rows within one file; this filters cell contents at name lookup.
...
When the country sub-slot cannot resolve (e.g. `"Denver, CO USA"`, `"Bogor, Atlantis"`), the tool flags `HEADQUARTERS_COUNTRY_INVALID` and the cleaner re-walks the cell more aggressively against the country-alias table. When a unique country resolves (`"USA"` inside `"CO USA"`), the cleaner proposes the resolved ISO-3 against the country sub-slot only — the city stays whatever the operator typed. When nothing resolves (prose like `"Irving, Texas (headquarters moving to Houston in June 2023)"`), the operator picks from the country dropdown in the review tab.
...
**Technical detail:** `MapperService.run` (`packages/mapper/src/mapper/mapper_service.py`) folds the matched candidate's name from `EnrichmentCode.MATCH_EXACT` / `MATCH_SINGLE_CANDIDATE` findings into the cell-override map ahead of cleaner fixes (an explicit `USER_CHOICE` correction still wins), so the resolved-row assembly and the `emit_entity_metadata_findings` Pass-2 `name_field` write carry the identical string; `_adopt_entity_spelling_for_routing_cells` then propagates the declaring cell's final spelling to the transactional rows that reference the entity by name. The invariant is pinned by `tests/unit/test_mapper_canonical_entity_ledger_sync.py`.
...
For SCD2 to actually merge a submission's contribution, the mapper must assemble one `metadata_*` row carrying the entity's `id_field` (e.g. `metadata_companies.eiti_id_company`) onto the `mapper_rows` channel. The emission is universal: `MapperService.emit_entity_metadata_findings` in `packages/mapper/src/mapper/mapper_service.py` writes the `EntityCreation.metadata_target.id_field` through the `emit_row` seam so every family participates uniformly — no per-family closure branching. The architectural test `tests/unit/test_every_minted_entity_id_lands_in_metadata_row.py` discovers every family in the built `family_registries` map with an entity-creating CohortField and asserts the emission contract — a future family that mints UUIDs but skips this step fails CI rather than silently importing every company with `eiti_id_company=''` (the empty-business-key SCD2 collision shape).
...
**Technical detail:** The importer reads the declaration UUID (`existence_key`) from `context.cohort_metadata`, where the detector stamps it via `stamp_cohort_identity`. If it is absent, the importer returns a single `MAPPING` finding with code `DECLARATION_UUID_MISSING` and writes nothing; the session lands in `ERROR_DATA`. A genuine database write failure further down (constraint violation, connection drop) is caught separately and produces a `DB_WRITE_FAILED` finding with the underlying exception text. Logic in `ImporterService.run` in `packages/importer/src/importer/import_service.py`. The UUID is a `uuid5(DECLARATION_NAMESPACE, f"{country_iso3}:{year}")` produced by the detector.
...
**Technical detail:** `_CORRECTIONS` in `packages/stores/eiti/src/eiti/parsing/transforms/declaration_value_corrections.py` — each entry is an `AbsoluteCorrection` or `MultiplierCorrection` naming `(iso3, reporting_start_year, table_name, where-conditions, field, value, rationale)`. An `AbsoluteCorrection` replaces the matched cell with `value` (a number, or a string for a text cell); a `MultiplierCorrection` scales the existing numeric cell by `value`. The transform is registered on `_V2_TRANSFORMS` and matched after `derive_about_metadata_from_country` (whose About identity it keys on). `DECLARED_VALUE_CORRECTED` is in `_SCHEMA_DEVIATION_PARSER_CODES`, so it lands in the observation channel, not the per-cell review gate — the corrected cell then validates normally and the file is not blocked on it.
...
**Technical detail:** The normalizer is `normalize_id_column` in `packages/stores/eiti/src/eiti/parsing/schemas/validation_helpers.py`, exposed as `NormID = Annotated[str, BeforeValidator(normalize_id_column)]` in the same file. Field declarations in `packages/stores/eiti/src/eiti/parsing/schemas/v2p0.py` and `v2p1.py` use `NormID` (sometimes combined with `NotAvailable` or `NotApplicable` in a union) for the ID-shaped columns: `id_number`, `company_id`, `legal_agreement_ref_num`, and `project_name`.
...
**Technical detail:** The gate lives in the review-submit path in `apps/api/src/api/session_endpoints.py`. It folds `context.findings` plus the freshly-submitted `request.corrections` through the one coverage authority — `core.findings.compose_findings` — and refuses to continue if any cell still carries an uncovered blocking finding. A finding is covered when the tool proposed a fix at its cell (or a covering token repair) or the user picked a value; a strict-typed source-only error (no candidates, no cleaner coverage) can only be fixed in the Excel file or escalated via FLAGGED. Uncovered findings result in a 422 response with the uncovered count.
...
**Technical detail:** The vocabulary lives in `packages/stores/eiti/src/eiti/validation_vocabulary.py` (catalog + per-number accepted names + accepted numbers). Enforcement is in `ValidationScoreRow` (`packages/stores/eiti/src/eiti/parsing/schemas/validation_data_v1.py`) via the `RequirementName`/`RequirementNumber` typed aliases; the blocking codes are `REQUIREMENT_NUMBER_UNKNOWN`, `REQUIREMENT_NAME_UNKNOWN`, `REQUIREMENT_PAIR_MISMATCH`. The 0.0 lift is `aggregate_validation_events` in `packages/stores/eiti/src/eiti/families/validation_data/__init__.py`; `req_id` resolution happens in the importer's FK step against `metadata_validation_requirements.requirement_number`.
...
**Technical detail:** The validation measure (`eiti.measures.validation`) partitions score rows into the 0.0 row / indicator rows / Standard requirement rows over the review-time overlay; indicator classification comes from `indicator_numbers` on `ValidationStatsConfig` (`packages/stores/eiti/src/eiti/submissions/stats.py`). The measure folds the validation scorecard server-side and the dashboard renders it via `renderValidationScorecard` in `apps/web_ui/components/dashboard.js`.
...
**Technical detail:** `core/findings/overlay.py` — `resolve_overlay` keeps `(rank, value)` per coordinate; `read_cell` compares the cell override's rank against the max token-override rank (cross-axis rule lives in `read_cell` only; exact-coordinate reads are per-coordinate). `ArchiveDropInvalidListTokenRule` co-emits per-token drops with its all-bad fill, and defers to invisible-valid tokens via the parser's `token_count` stamp (the typed `ParsingError.token_count` field on per-token findings, computed with the field's own segmenter), emitting the cell-covering `LIST_RESOLVED_BY_TOKEN_DROPS` in that branch so the gate stays covered. See BUG-060/061/062.

<!-- scenario: an operator maps a headquarters cell's country in the corrections manifest while the cleaner also resolves it; topic: correction-precedence -->
## A correction to a headquarters cell's country competes with the cleaner's own resolution, and the operator wins

**Situation:** The headquarters cell is compound — it carries a city and a country. When its country can't be recognised, two corrections can target the same country: the cleaner's automatic re-walk of the cell against the country-alias table, and an operator's corrections-manifest entry mapping the cell to a country. An operator who supplies the manifest entry expects their choice to be what imports.

**Decision:** Both corrections land on the country sub-slot and compete on the same precedence ladder every other cell uses; the operator's manifest entry (which ranks above an automatic cleaner fix) wins, and the mapper writes the operator's country. There is no separate whole-cell-vs-country arbitration, because a whole-cell correction and a country correction can never both exist on one headquarters cell: a cell carries a single validation finding, and every correction responding to it inherits that finding's grain. A blank or sentinel headquarters cell (the whole location is absent) is a whole-cell correction; an unrecognised country is a country-only correction; the two are mutually exclusive.

**Rationale:** The two corrections used to land at different addresses — the operator's at the whole cell, the cleaner's at the country sub-slot — so they never compared, and the cleaner's automatic value silently overwrote the operator's manifest choice (and the cross-sheet consistency check crashed when both were present). Anchoring both at the country the moment the cell's country is what failed to validate makes the operator's precedence hold the same way it does on every other cell.

**Technical detail:** The parser stamps `field_path=("country",)` on the `HEADQUARTERS_COUNTRY_INVALID` finding (`HeadquartersCountryError` in `packages/stores/eiti/src/eiti/parsing/schemas/exceptions.py`; carried through `packages/parser/src/parser/validation/row_validator.py`). The code-agnostic `ManifestNormalizationRule` and the `HeadquartersCountryResolutionRule` in `packages/cleaner/src/cleaner/rules.py` both propagate the finding's `field_path`, so their corrections share the country coordinate and `resolve_overlay` picks the higher tier. A blank/sentinel headquarters cell routes through `CELL_BLANK` / the sentinel codes, which carry no `field_path`, so it stays a whole-cell correction. See BUG-061 for the sibling cell-vs-token axis.

<!-- scenario: an archived v1 file leaves the About-sheet reporting currency blank; topic: import-behavior -->
## A v1 file with a blank reporting currency is filled from a report-sourced archive map (archive mode) or blocks (strict mode)

**Situation:** Some v1 SDF submissions leave the About sheet's "reporting currency" cell blank. Because a declaration's reporting currency is required — every monetary figure is denominated in it — a blank cell blocks the whole file at review for a value the operator cannot recover from the workbook itself.

**Decision:** Under archive-upload mode (which the corpus tester opts into while importing the historical corpus), the tool fills the blank cell from a curated map keyed by country **and year**, where each entry is the ISO-4217 code that cohort actually reported in, read from that cohort's own published EITI report. A country-year absent from the map keeps its blank cell blocking — the fill never guesses. Outside archive mode the blank always blocks for operator review. Either way the currency is never silently invented: it is filled from a report-verified value or the file stops.

**Rationale:** The reporting currency is not a stable property of a country that could be defaulted from the country name. Across a single country's own filings it varies by year and carries operator errors — Cameroon's SDFs declared XOF, XAF, and USD in different years (XOF is wrong for a Central-African-CFA country); the DRC filed in USD most years but CDF in 2008; Chad's oil figures are in USD, not its XAF national currency. The one machine-readable cross-source — the frozen v1 API extract — denominates every value in USD (it converted each country's figures via an exchange rate), so it records the *presentation* currency of the export, not what the country reported in; filling from it would relabel every blank file as USD. The country-year's published report is the only authoritative source, so each map entry is verified against it. Keeping the behaviour in a parser transform (not a downstream service) and emitting a finding per fill preserves the standing rule that archive mode shapes data upstream and stays auditable.

**Technical detail:** The map lives in `packages/stores/eiti/src/eiti/v1_reporting_currencies.json` (loaded as `V1_REPORTING_CURRENCIES`, keyed `ISO3_YEAR`), parallel to the `V1_EXCHANGE_RATES` archive map. The parser transform `fill_v1_reporting_currency` (`packages/stores/eiti/src/eiti/parsing/transforms/reporting_currency_archive.py`) runs pre-validate, resolves the cohort key from the About row's `country` + `start_date` year, fills a blank `reporting_currency`, and emits one `V1_REPORTING_CURRENCY_FILLED_FROM_ARCHIVE` (`SCHEMA_DEVIATION`) finding per fill. It is gated on the legacy-archive import mode and never overwrites an operator-supplied value. Values are validated by the same `CurrencyCell` adapter as a directly-declared cell (whitespace-normalised, then coerced to the `CurrencyCode` enum, which includes the withdrawn codes archive years need — e.g. Mauritania's pre-2018 `MRO`).

<!-- scenario: a file names a commodity generically ("Minerals", "Metallic ores") rather than a specific one; topic: commodity recognition -->
## A generically-named commodity resolves to the HS "not elsewhere specified" bucket; a truly unidentifiable one is kept as disclosed, unattributed

**Situation:** Some declarations name a commodity non-specifically — "Minerals", "Metallic ores", "Rocks" / "Roches", "Oil products" — rather than a specific mineral, ore, or fuel. The recognizer keys on the commodity's 4-digit HS heading, and these generic terms match no specific heading, so previously every one blocked the file at review as an unrecognised commodity.

**Decision:** A generic *mineral/ore/fuel* label resolves to the HS heading that classifies exactly that unspecified case: "Minerals" / "Rocks" → 2530 (*Mineral substances not elsewhere specified*), "Metallic ores" → 2617 (*Other ores and concentrates*), "Oil products" → 2710 (*Petroleum oils, excluding crude*). The file then imports with the commodity honestly recorded as the unspecified bucket. A label that carries no catalogued commodity meaning — bare "Other" / "Others" (also a sector, agency-type, and project-phase value), rock "samples", a real mineral not yet catalogued, or a non-mineral production row — is **not** aliased to a bucket, but under archive-upload mode it no longer blocks either: the disclosed label is kept as-is and the file imports. The commodity-shaped row lands in the clean tier with `commodity_id=''` — its production value stays in the analytical data (in section and by-sector totals) but is excluded from every per-commodity view by construction, so it is never mistaken for an attributed commodity. Catalogue the HS later and the same row attributes with no reload. Outside archive mode, an unresolvable commodity still blocks for operator review.

**Rationale:** Two different situations need two different answers. When the source named a *category* HS itself defines — "Metallic ores", "Minerals" — recording it in the corresponding *not-elsewhere-specified* heading loses no fidelity the data ever had, and lands the value in clean data attributed to that bucket. When the label carries no catalogued commodity meaning at all, inventing an "other commodity" dimension for it would be a fake bucket that pollutes the commodity catalog — but blocking the whole file over it (in an archive backfill with no operator present) throws away every other row too, and dropping the row deletes a real disclosed production value. Keeping the disclosed label threads the needle: the file imports, the value stays analytical but unattributed (`commodity_id=''`, invisible to per-commodity rollups), and no meaningless commodity entity is minted. A reported-but-uncatalogued mineral is analytically extractive even when unnamed, so preserving it — rather than discarding it as *Not available* did — keeps the record faithful and lets later catalogue work attribute it.

**Technical detail:** The generic *mineral* terms are aliases in `COMMODITY_TRANSLATIONS` (`packages/stores/eiti/src/eiti/sdf_vocabulary.py`) on the `2530` / `2617` / `2710` canonicals, resolved by the cleaner's `resolve_commodity_alias` arm; bare `Other` is deliberately excluded from the `2617` alias set (documented inline) because it collides with the `Other` canonical key in `SECTOR_TRANSLATIONS`, `AGENCY_TYPE_TRANSLATIONS`, and `PROJECT_PHASE_TRANSLATIONS`, and the `fold_commodity_aliases` ambiguity rule would drop a form resolving to more than one canonical anyway. Everything the alias arm cannot attribute is then caught by `ArchiveKeepUnidentifiedCommodityRule` (`packages/cleaner/src/cleaner/rules.py`), gated on `accept_legacy_archive_imports` and run immediately after the alias rule, which keeps the disclosed label for the remaining `COMMODITY_UNKNOWN` cells and logs `CleaningCode.COMMODITY_KEPT_UNIDENTIFIED` (a fixing code, so the review gate counts the cell covered). The commodity-shaped label survives, so the production value lands in the clean tier unattributed (`commodity_id=''`, which every per-commodity and per-sector view excludes by construction) rather than being dropped — catalogue the HS later and the same row attributes with no reload. `test_commodity_recognition.py`'s alias round-trip pins each generic term to its heading; `test_keep_unidentified_commodity_rule.py` pins the archive-mode disposition.

<!-- scenario: the operator left the template's generic `Commodity 1`…`Commodity N` slots unfilled; topic: commodity recognition -->
## An unfilled `Commodity N` template slot is dropped, not recorded as a commodity

**Situation:** The v1 SDF template seeds its Part-2 commodity table with generic `Commodity 1` … `Commodity N` placeholder rows for the operator to overwrite with real names. Operators routinely leave the surplus slots unfilled — the label stays `Commodity 3` and the value cell is blank or a `Not available` / `Not applicable` sentinel.

**Decision:** An unfilled slot is treated as template scaffolding, not a disclosure: the file imports, but the row is dropped from the clean analytical tier — it does not appear as a commodity, not even as an unattributed `commodity_id=''` row. A slot that names nothing discloses nothing.

**Rationale:** This is the deliberate opposite of the keep-shaped disposition above. A commodity-shaped row whose commodity is merely *uncatalogued* is kept in the clean tier with `commodity_id=''`, because a reported-but-uncatalogued mineral is analytically extractive even when unnamed. An unfilled slot carries no commodity name at all, so keeping it would seed a phantom "some commodity, value not available" row for a slot the operator never used. `recognize_commodity` reads both cases as an uncatalogued miss, so the distinction is drawn by a dedicated predicate rather than left to HS recognition.

**Technical detail:** `is_unfilled_commodity_slot` (`packages/stores/eiti/src/eiti/commodity_recognition.py`, matched on the metric-stripped base `^commodity \d+$`, case-insensitive) is the single source both consumers read: the parser's `_validate_v1_sub_item` passes a slot through unflagged (so it never blocks, in strict or archive mode), and the Part-2 finalizer excludes a slot from `is_commodity`, so it lands with a NULL `indicator_shorthand` the identity-gated `INDICATOR_VALUES_PROJECTION` drops. Independent of `accept_legacy_archive_imports` — a slot names no commodity in any mode. `test_commodity_recognition.py` pins the predicate; `test_part2_finalizer.py` pins the drop.

<!-- scenario: an operator entered a monetary amount in European number format (comma decimal, space thousands); topic: import-behavior -->
## An amount in European number format imports at its true magnitude

**Situation:** The EITI template is English-formatted (`1,234.56` — comma thousands, dot decimal), but operators, especially in Francophone countries (Niger, Gabon), enter amounts in the European convention: a comma decimal (`2981,63`) and space or non-breaking-space thousands (`157 256,93`). Depending on the field, such a cell either blocked the file as an invalid number or was read by an English-only comma strip as `298163` — a silent 100× corruption with no signal that anything went wrong.

**Decision:** The tool recognizes both conventions and imports each amount at its true magnitude. It reads the English template by default, and the European convention when the cell carries an unambiguous tell — a comma followed by at most two decimal digits, space/NBSP thousands in three-digit groups, or a leading `0,` that cannot be an English thousands group. An amount that fits neither grammar — a contradiction like `1 234,567`, a mixed-separator `50,486,312.534.10`, or dot-thousands `3.599.322.893` — is left unparsed and blocks for operator review. A genuinely ambiguous bare `1,234` reads as the template's meaning (one thousand two hundred thirty-four). The tool never silently imports a guessed magnitude.

**Rationale:** A misread amount is the worst kind of data loss — the figure is present and plausible but off by 100×, so a data user sees a wrong total with no signal anything went wrong. Anchoring European recognition on strict tells, rather than "any comma is a decimal", keeps the English template — the format most files already use — parsing exactly as before, including negatives, while recovering the European files that previously blocked or corrupted. Leaving contradictions and unsupported shapes unparsed turns a would-be silent corruption into a visible block, which is the safe direction.

**Technical detail:** One grammar, `core.numeric_norm.parse_locale_number`, is the single place a cell string becomes a number. Every amount field carries the `LocaleFloat` type (`eiti.parsing.schemas.validation_helpers`) that applies it, and the three sites the field type cannot reach — the family-assembly value split (`eiti.families._resolved_assembly._split_numeric`, which covers v1's str-primary `entry` cell), the exchange-rate fallback, and the compound-fanout numeric probe — consume the same function. `tests/unit/test_arch_single_numeric_grammar.py` fails on any other comma-stripping numeric parser (`x.replace(",", "")` / `.replace(",", ".")`, in any wrapper) and on any float-primary amount field annotated with a bare `float`, so the one-grammar property holds by construction. A recognized amount imports with no finding (the normalization is lossless, `2981,63 ≡ 2981.63`); an unrecognized one takes the existing invalid-datatype path.

<!-- scenario: an operator wrote "N/C" or "NC" (non communiqué) in a cell; topic: import-behavior -->
## A cell reading "N/C" or "NC" is imported as Not available, not kept as literal text

**Situation:** Francophone SDFs (Niger, Chad, Togo, Mali, Burkina Faso) abbreviate *non communiqué* — "not communicated" — as `N/C` or bare `NC`, across company-id, stock-exchange-listing, revenue-reporting, and Y/N project columns. Previously `N/C` blocked typed/enum fields as an invalid datatype (holding up the whole file), while bare `NC` was kept as the literal string on free-text fields — so eleven companies could share a meaningless id of "NC".

**Decision:** Both `N/C` and `NC` are recognized as the *Not available* sentinel. On typed/enum and general value fields they standardize to Not available (so the file is no longer blocked on them, and no company shares a bogus literal id). On an **entity-identity field** (a company / project / agency name), `N/C` marks a *withheld identity*, not the value "Not available" — see the entry below on withheld entities. New Caledonia is unaffected — EITI reports it under its ISO-3 code `NCL`, and `NC` never appears as a country code in the corpus.

**Rationale:** `NC` / `N/C` is unambiguously a data-absence marker on every field it appears on — a company id, a stock-exchange listing, a revenue figure, or a Yes/No answer is never legitimately the two letters "NC". Recording it as Not available is more honest than keeping a bogus literal, and it fixes entity resolution: several companies sharing a fabricated id "NC" would mis-group. On an identity field the same honesty means the cell names no entity — so it is treated as a withheld identity rather than minting a literal "N/C" company or project.

**Technical detail:** the `nc` / `n/c` family lives in the store's absence vocabulary (`eiti.absence_vocab`, injected into `core.absence_vocab`; see the ADR-009 update on the vocabulary's home). On value fields the cleaner's `StandardizeNotAvailableRule` / `WrongSentinelCorrectionRule` resolve them to Not available; on entity-identity fields `core.entity_norm.is_withheld_marker` recognises them, and the withheld-entity path below takes over. The corpus drift-detector regex `_LIKELY_SENTINEL_RE` (`tests/unit/test_corpus_no_sentinel_variants_on_str_unions.py`) tracks the `nc` family against future corpus drift.

<!-- scenario: an operator withheld an entity's name but disclosed its money; topic: import-behavior -->
## A withheld entity name is imported as an anonymous entity, not a literal name — and, for a project, split per operator

**Situation:** An operator sometimes suppresses an entity's identity while still disclosing its money — a company, a project, or an agency named only `N/C`, `Non communiqué`, `Not available`, `withheld`, `confidential`, and the like. Previously such a cell either minted a literal garbage entity (eleven "projects" all named `N/C`) or, once recognised as a sentinel, collapsed every withheld entity in a report onto one shared "Anonymous" bucket — so their disclosed money could not be told apart.

**Decision:** A withheld name on an entity-identity field is never imported as a literal entity. In every mode it names no entity. On an **archive** upload it becomes an explicit anonymous entity so the row's money survives (a company → an *Anonymous company*, an agency → an *Anonymous agency*, a project → an *Anonymous project*). For a **project** that also discloses an operator, the anonymous project is **disaggregated by that operator**: two withheld projects operated by different companies in the same country become two distinct anonymous projects, and the same operator's withheld project in the same country is the *same* entity across every report (so its money accumulates rather than fragmenting per file). A withheld project with no disclosed operator, and every withheld company/agency, stays a single per-report anonymous entity. On a **live** submission a withheld name is treated as an absent identity (like "Not available") and never blocks the file.

Typed markers behave the same as a blank cell here: on the v2 identity columns that accept "Not available", an operator-typed `N/C` produces the same *Anonymous* outcome a blank cell already did, rather than the value "Not available". The "Not applicable" family is excluded — a not-applicable project column (a company-level revenue row that is not project-level) is not a withheld identity and mints no anonymous entity.

**Rationale:** The operator disclosed real money against a real entity whose name they chose to withhold; preserving that money is more faithful than dropping the row, and collapsing every withheld entity onto one bucket destroys the distinction the disclosed operator makes recoverable. Disaggregating a project by its operator is the one axis the source actually provides, and doing it with a deterministic id (the operator's identity in the reporting country) makes the same withheld project converge across reports without any manual matching. The disaggregation only ever *adds* project-level attribution — a project whose operator can't be resolved keeps its money on the company, never loses it.

**Technical detail:** recognition is `core.entity_norm.is_withheld_marker` (the store's `nv | entity_extras` vocabulary, excluding the `na` family); the archive cleaner mints the anonymous placeholder (`ArchiveMintAnonymousEntityRule`). Project disaggregation is online: the store enricher's withheld-project fold emits `EnrichmentCode.MATCH_WITHHELD` carrying `core.ids.withheld_entity_id(operator, country, entity_type=project)`, and the mapper emits its metadata row and attaches the clean row's project key by coordinate. No offline manifest is involved; the same-operator-same-country convergence and cross-report dedup both fall out of the deterministic id.

<!-- scenario: an operator wants to see what the importer found and changed for a declaration, long after the import ran; topic: import-behavior -->
## The importer keeps a permanent record of every finding from a confirmed import

**Situation:** When you import a submission and confirm it, the importer records what it found and changed — every flagged cell, every entity it matched or minted, every correction. Until now those records lived only in a temporary session cache and were lost when the session expired, so after the fact there was no way to ask "what did the importer observe and decide for this declaration?"

**Decision:** The importer now keeps a permanent, queryable record of the complete set of findings from every confirmed import, tied to that import. The record is written at the moment the import commits and only for imports that actually commit — if an import fails, it leaves no findings behind.

**Rationale:** This lets you, and later tooling, see for any declaration exactly what the importer observed and decided at import time — and, in a later step, who corrected what and why — long after the import ran. Keeping the whole set (not just the cell-addressed findings) preserves the records that can't be reconstructed from the source file alone, such as which existing entity a newly-imported name was matched to.

<!-- scenario: an operator realises they picked the wrong entity or value at review for a declaration that already imported, and wants to fix it without re-uploading; topic: import-behavior -->
## Reopening a committed declaration overwrites it, and the audit trail records what changed, by whom, and why

**Situation:** After a declaration is imported, an operator sometimes needs to change a choice they made at review — pick a different company for a cell, revise a value. The imported data is already committed, and re-uploading the whole file to fix one choice is heavy-handed.

**Decision:** An operator can *reopen* a committed declaration. Reopening rebuilds the original review page from what the importer stored — the same questions, with the operator's previous answers already filled in — so they revise just the choice they want to change and re-confirm. Re-confirming **overwrites** the committed declaration with the corrected result. Reopen can be started from the web UI's Manage tab — a per-row Reopen button that takes the operator straight to the reconstructed review page — or from the command line (`eiti reopen`), which prints a link that opens the same page in the browser. Nothing changes in the database until the operator confirms in the browser. If the declaration changed in the meantime — someone else imported or corrected it after you reopened — your re-confirm is refused with a clear message and you reopen again against the current version, rather than silently overwriting their change.

**Rationale:** The correction flows through exactly the same path a first import does — the answer drives the same downstream computation — so the corrected value is as durable as any imported value, not a patch that a later refresh could wipe. Because reopening overwrites, the audit log records each changed answer as its own entry: what the value was, what it became (entity picks show the company name, not an internal id), who made the change, when, and the reason they gave at confirmation. A reopen that changes nothing leaves no audit entry. One consequence to know: if you change an entity pick, the entity you originally chose stays in the reference registry even though this declaration no longer points at it — the same as when you delete a declaration.

<!-- scenario: an operator working from the terminal wants to see a session's pre-import statistics without opening the browser; topic: cli -->
## The CLI can show a submission's statistics panel

**Situation:** The web dashboard shows a session's statistics — revenue totals and gap, currency and rate, commodities, coverage, operators, the validation scorecard. An operator working from the terminal had no equivalent view.

**Decision:** `eiti review --view stats --file <session>` renders the same statistics as a Rich panel in the terminal, reading the identical figures the dashboard shows. It fetches the session's statistics from the server and renders them; it computes nothing locally.

**Rationale:** The statistics have one definition, computed server-side, so the CLI and the web dashboard show the same numbers by construction — a terminal operator sees exactly what a browser operator sees, with no second implementation to drift.

<!-- scenario: an operator wants to find one previously-imported cohort among many to preview, reopen, or delete; topic: import-behavior -->
## The Manage tab filters imports by category, and each category offers only the dimensions that apply to it

**Situation:** After many imports, the list of previously-imported cohorts is long, and an operator needs to find a particular one to preview, reopen, or delete it. The categories differ: a Summary Data File is scoped to a country and a year; a Company Assessment is scoped to a year but has no country at all.

**Decision:** The Manage tab (previously labelled "Delete") lets an operator search imports by name and filter them. Filtering is two-level: first pick a category from the **Category** dropdown (Summary Data Files, Company Assessment, Validation Data), then narrow by the dimensions *that category actually has*. Summary Data Files and Validation Data can be filtered by country and year; Company Assessment, which is not country-scoped, offers a year filter only — a Country filter never appears for it. Each import row leads with its human cohort name, with the technical identifiers demoted beneath.

**Rationale:** Different submission categories describe different things, so one flat filter bar would offer dimensions that don't apply — a country filter is meaningless for a category that has no country, and greying it out would just be clutter that says "this control does nothing here." Instead each category declares its own filterable dimensions, so an operator only ever sees a filter that will actually narrow the list, and a category added later brings its own filters with it without any change to this screen.

## An operator can correct an extracted value on fields the family marks editable

**Situation:** Review lets an operator pick among candidates the pipeline offered (an entity match, an enum value), but some cells are a plain figure the parser read straight from the file — a revenue amount, for instance. When such a value is wrong and no auto-fix applies, the only remedy has been to edit the source spreadsheet and re-upload the whole file, even for a single number.

**Decision:** A family can mark a data field editable. On a marked column the Details data table shows an inline editor down every cell of that column: the operator types a corrected figure (or the canonical "Not available"), and it is submitted as the same correction a candidate pick writes. The edit is checked against the field's own type before it is accepted — a value that isn't a valid figure for that field is refused with a clear message, so a typo can't land as data. Editability is opt-in per field: a family that marks nothing keeps every cell read-only, and marking a value field does not make entity-name fields editable (re-identifying an entity is a separate, later capability). On a marked column, a cell that would otherwise read "fix in source" instead becomes correctable in-app — the editor is the remedy. The correction persists exactly like any review answer and reproduces through reopen.

**Rationale:** The edit rides the same path as every other review correction — it enters as the top-precedence value contribution and flows through the same downstream computation — so a corrected figure is as durable as any imported value, not a patch a later refresh could wipe. Type-checking the edit against the field itself (rather than a hand-written rule per field) means every editable field is guarded by the same schema the parser already validates against, so the guard cannot drift from the field's real type. Keeping editability an explicit per-field mark, rather than making every cell editable, means the affordance appears only where a family has decided an operator override is appropriate.

<!-- scenario: an operator bulk-imports historical Summary Data Files and wants to know what the importer does with project-field cells that hold something other than a real project name; topic: import-behavior -->
## Archive imports correct project-field cells that aren't real projects

**Situation:** Across the historical Summary Data File corpus, more than half of the free-form project-name cells do not hold a clean project. Operators typed a company or operator name into the project field, concatenated several projects (or several companies) into one cell, fused a project with its affiliated company, entered an aggregate label ("all mines"), left a placeholder, or entered an administrative reference — a Netherlands permit-fee code (`APP LEGES …` / `APP RETRIBUTIES …`) or a Togo/Guinea ministerial permit-decree number. Left as-is, each of these mints a bogus "project", which pollutes the project list and the deduplication that runs over it.

**Decision:** When you import in archive mode, the importer applies a frozen, per-cell verdict — built by a reviewed classification of every historical project-name cell — plus a recognizer for the fee/decree reference patterns. What it does depends on where the cell sits:

- On a project **declaration** sheet (the Projects list), the cell defines a project, so it is repaired: a misfiled company (single or a list) moves to the row's affiliated companies and the project becomes an *Anonymous project*; a bundle of several projects is split into one project row per project (the combined production figure becomes "Not available", since it can't be split per project); a project fused with a company keeps the project and adds the company as an affiliate; an aggregate label becomes the "Not disaggregated" bucket; and a placeholder, withheld, or administrative-reference cell becomes an *Anonymous project*.
- On a **payment** row, the same cell is only a reference tagging where the money went, so it is never turned into a project. Any non-project value there is cleared to "Not available": the payment still imports and still flows to its company and government entity, it simply carries no project attribution — and no project, anonymous or otherwise, is created from it.

The reviewed verdict is the only thing that acts, and it is conservative. A cell the reviewers never adjudicated is left exactly as submitted — even one that superficially looks like an administrative reference (a Guinea concession whose name embeds a decree number, a Dutch fee line that ends in a real gas-field name), because those are precisely the shapes no rule can separate from a genuine artifact, which is why the human review exists. A handful of cells the reviewers mislabelled (two Guinea decree-reference lists recorded as project bundles they could not split) are corrected explicitly, the same way a provably-wrong archive value is corrected. Cells the reviewers could not agree on are left exactly as submitted. Outside archive mode — a normal single-file upload — none of this applies: the frozen verdict never reshapes a live submission, which blocks for operator review in the usual way instead.

**Rationale:** A company, a bundle, or a fee code sitting in the project field is a data-entry error that only the source corpus can resolve, so — like the other archive-specific corrections — the fixes are pinned to the specific historical cells rather than loosened into the schema, keeping live uploads strict. Repairing a declaration but only NV-ing a payment reference follows what each position means: a declaration is *supposed* to name a project, so the honest repair recovers or anonymizes one; a payment reference merely points at a project, so when it points at nothing real the honest result is "no project", never an invented one — and never a fanned-out reference that would multiply the payment across several projects. The reviewed verdict is the sole authority: there is no pattern rule guessing at cells no one adjudicated, because on these shapes a guess destroys as many real projects as artifacts it would catch — a genuinely new administrative reference is handled by re-reviewing the corpus, not by a regex.

<!-- scenario: an operator submits a Summary Data File whose About-sheet reporting period is blank, unreadable in its language, or spans an impossible range, and wants to know whether it still imports; topic: import-behavior -->
## A file whose reporting period can't be trusted is repaired or flagged, never mis-cohorted

**Situation:** A Summary Data File's identity — which country-year declaration it belongs to — is keyed on the reporting period's end year, read from the About sheet. That cell can be unreadable (a localized file whose "End Date" wording the tool didn't recognize), or provably wrong (a cell spanning two calendar years, so the report's true year is ambiguous). Previously such a file was silently filed under its *start* year, landing on the wrong declaration.

**Decision:** The end year is no longer allowed to silently fall back to the start year when the period is implausible. A reporting period longer than two calendar years is treated as a bad cell. If a curator has recorded a verified year for that specific file (keyed on its filename), the file is corrected to that year and imports normally. If not, the file is **blocked** with a clear error rather than imported under a guessed year — it surfaces for a curator to inspect. A normal file — including a legitimate transitional fiscal year that combines two consecutive periods, such as a report covering a country's move from a March to a December fiscal-year end — is unaffected. Separately, the localized "End Date" wording that used to go unread (Russian and Spanish first-generation files) is now recognized, so those files key on their real end year without any repair.

**Rationale:** Filing a declaration under the wrong year is a silent data-integrity error: it splits a country's disclosures across two years and reconciles against the wrong EITI extract. Blocking an untrustworthy file is honest — the operator learns the period is bad instead of discovering a misfiled declaration later — and a per-file curator repair (rather than a loosened rule) keeps the correction pinned to the one file that needs it, exactly as the archive project-cell corrections are. The two-year bar sits above the longest real transitional fiscal year — a combined two-period report reaches nearly two years but never exceeds it — so no genuine report is caught, while a typo'd multi-year span still is.
