...
**Decision:** Both `N/C` and `NC` are recognized as the *Not available* sentinel. On typed/enum fields they standardize to Not available (so the file is no longer blocked on them); on free-text fields the literal `NC` is replaced by Not available. New Caledonia is unaffected — EITI reports it under its ISO-3 code `NCL`, and `NC` never appears as a country code in the corpus.

**Rationale:** `NC` / `N/C` is unambiguously a data-absence marker on every field it appears on — a company id, a stock-exchange listing, a revenue figure, or a Yes/No answer is never legitimately the two letters "NC". Recording it as Not available is more honest than keeping a bogus literal, and it fixes entity resolution: several companies sharing a fabricated id "NC" would mis-group, whereas Not available lets them resolve by name. The change is data honesty rather than coverage — it flips no files and caused no regressions across the full v1+v2 corpus.

**Technical detail:** `"nc"` and `"n/c"` are registered in `NV_VARIANTS` (`core/diagnostics.py`); the cleaner's `StandardizeNotAvailableRule` resolves them on typed/enum fields (which reach `DATATYPE_INVALID`), and the sentinel-lookalike `BeforeValidator` → `WrongSentinelCorrectionRule` resolves them on str-union fields. The corpus drift-detector regex `_LIKELY_SENTINEL_RE` (`tests/unit/test_corpus_no_sentinel_variants_on_str_unions.py`) was widened from `n[/.\s]*a` to `n[/.\s]*[ac]` so the `nc` family is tracked against future corpus drift.
