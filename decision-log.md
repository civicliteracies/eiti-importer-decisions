...
**Technical detail:** `_make_part5_references_check` in `packages/shared/src/shared/families/sdf.py` builds the check from per-version table/column configs declared alongside each submission's crosschecker registration. The roles produce `UNREGISTERED_COMPANY`, `UNREGISTERED_AGENCY`, and `UNREGISTERED_PROJECT` finding codes. Sentinel strings ("Not available", "Not applicable") are filtered via `is_real_string` (`packages/shared/src/shared/diagnostics.py`) before set membership so they never produce 'unregistered company "Not available"' findings.
